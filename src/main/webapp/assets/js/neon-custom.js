...
function fit_main_content_height()
{
	...
	if(public_vars.$sidebarMenu.length && public_vars.$sidebarMenu.hasClass('fixed') == false)
	{
		...
		if(typeof fit_mail_container_height != 'undefined')
			...
			//$('#cards_iframe').css('height',doc_height-60);//calc height for iframe
		}
...
function setup_horizontal_menu()
{
	...
	//菜单单击事件
	public_vars.$mainMenu.find('a').click(function(e){
		var $this=$(this),
			action=$this.attr('action'),
			$card_iframe=$('#cards_iframe');
			if(action){
				$card_iframe.attr('src',action);
				var $pli=$this.parents('li.root-level.has-sub');
				if($pli.length){
					$pli.find('ul li').removeClass('active');
					$this.parent().addClass('active');
				}
			}
			return false;
	...
	$('#cards_iframe').css('height',$('body').height()-145);
}
